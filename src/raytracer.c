...
** Last update Wed Mar 15 10:39:19 2017 Martin Van Elslande
*/
...
sfVector3f		set_light_vector(t_obj *light, sfVector3f intersection)
{
  sfVector3f		light_vector;

  light_vector.x = light->coords.x - intersection.x;
  light_vector.y = light->coords.y - intersection.y;
  light_vector.z = light->coords.z - intersection.z;
  return (light_vector);
}
				       ...
				       int id, t_obj *obj)
{
  ...
  d_vec = set_light_vector(light, p);
  while (obj)
    {
      if (obj->id != id)
	{
	  if (obj->type == 1 &&
	      (k = u_intersect_plane(p, d_vec, obj)) >= 0.0f && k < 0.999f)
	    return (0.1f);
	  else if (obj->type == 2 &&
		   (k = u_intersect_sphere(p, d_vec, obj)) >= 0.0f && k < 0.999f)
	    return (0.1f);
	  else if (obj->type == 3 &&
		   (k = u_intersect_cylinder(p, d_vec, obj)) >= 0.0f && k < 0.999f)
	    return (0.1f);
	  else if (obj->type == 4 &&
		   (k = u_intersect_cone(p, d_vec, obj)) >= 0.0f && k < 0.999f)
	    return (0.1f);
	}
      obj = obj->next;
    }
				    ...
				    t_obj *objhead, sfVector3f intersection)
{
  ...
  light_vector = set_light_vector(light_obj, intersection);
